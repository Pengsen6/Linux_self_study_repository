...
\subsubsection{消融实验的粒度局限与改进方向}

当前消融实验将DyGAT整体替换为标准GAT以评估其综合贡献。但从组件级分析的角度，DyGAT由三个可分离的子组件构成——空间注意力（式\ref{eq:spatial_attn}-\ref{eq:alpha}）、时序注意力（式\ref{eq:temporal_attn}-\ref{eq:temporal_out}）与动态边权重（式\ref{eq:edge_weight}）。三者通过Hadamard积（式\ref{eq:edge_final}）融合，但各自的独立边际贡献尚未在消融实验中被量化。这一粒度不足是当前实验设计的局限——标准GAT的替换同时移除了三个组件，导致173.6\%的性能退化是三者的联合效应，无法确定哪个组件贡献最大。建议的改进方案包括：(i) 分别移除动态边权重（仅用二值邻接$adj_{ij}$）、移除时序注意力（仅使用当前时间步特征）、以及将空间注意力替换为均值池化，以建立组件级贡献排序；(ii) 通过上述精细化消融，验证``空间注意力+动态边权重的互补效应''这一核心设计假设。鉴于当前实验条件限制，该精细化消融留待后续版本补充。

\subsubsection{轨迹加权系数$\lambda$的选取讨论}

$\lambda=2.0$（冲突episode损失权重）是本文训练机制的关键超参数。当前实验仅对比了$\lambda=2.0$（完整模型）与$\lambda=1.0$（无加权消融）两种配置，$\lambda$的最优取值及其敏感性尚未被系统验证。从理论角度分析，$\lambda$对模型性能的影响预期呈倒U型曲线：过小的$\lambda$（接近1.0）不足以使冲突样本获得充分的梯度信号；过大的$\lambda$（如3.0以上）可能导致模型对冲突episode的过拟合，使策略过度保守（过度减速以规避任何潜在风险），反而因速度不一致诱发新的冲突构型。建议在$\lambda \in \{1.0, 1.5, 2.0, 2.5, 3.0\}$范围内进行敏感性实验，以确认$\lambda=2.0$的最优性并评估模型对该超参数的鲁棒性。该实验留待后续版本补充。
...
将安全距离违反按最小间距$d_{\min}$与安全距离$d_{\text{safety}}$的比值进行三级分层：

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{轻微违规（$0.8 \leq d_{\min}/d_{\text{safety}} < 1.0$）：}飞行器间距接近但仅略低于安全距离阈值，制动或机动可迅速恢复安全间隔。此类违规通常由传感器噪声或短暂的速度波动引起，在低空交通的初期运营阶段可能是可容忍的。
    \item \textbf{中等违规（$0.5 \leq d_{\min}/d_{\text{safety}} < 0.8$）：}飞行器间距显著低于安全距离，存在一定的碰撞风险，但两机仍有足够的反应时间与机动空间。此类违规通常由不完美的协同决策引起。
    \item \textbf{严重违规（$d_{\min}/d_{\text{safety}} < 0.5$）：}飞行器间距低于安全距离的一半，已进入接近碰撞（near mid-air collision, NMAC）的风险区域，在任何运营标准下均为不可接受事件。
\end{enumerate}

当前实验未按上述层级统计冲突严重程度分布——这是实验设计的局限。但基于冲突次数与CFCR之间的数量关系可做初步推断：如果31\%的``有冲突通过''episode中的冲突主要为轻微违规（由风扰/噪声触发的瞬时安全距离违反），则系统在实际部署中的安全风险可控；如果这些episode中包含显著比例的中等或严重违规，则CFCR = 68.4\%远未达到部署门槛。建议后续实验统计$d_{\min}$的完整分布，并报告``严重违规率''（$d_{\min} < 0.5 \times d_{\text{safety}}$的episode比例）作为比CFCR更严格的安全指标。

\subsubsection{CFCR的部署可接受性讨论}

对于城市低空eVTOL运营，美国FAA和欧洲EASA目前尚未发布量化的安全性能指标要求。参考民航领域的既有标准——例如终端区的碰撞风险目标为$10^{-7}$每飞行小时——以及无人机交通管理的研究文献，CFCR在部署前至少应达到95\%以上（即每20个航班中至多1个发生冲突）。从68.4\%到95\%的提升路径包括：

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{引入互补安全层：}将本文的学习型策略与速度障碍法（ORCA）、控制障碍函数（CBF）等形式化安全保证方法相结合。MARL策略负责效率优化与常规协同，ORCA/CBF作为底层安全网提供实时冲突避免的硬性保证。这种``学习优化+形式化安全''的双层架构有望将CFCR提升至接近100\%。
    \item \textbf{改进奖励函数设计：}当前安全奖励（式\ref{eq:rsafe}）对严重违规与轻微违规的惩罚梯度差异不够陡峭。增加严重违规的指数惩罚系数（如从$-50$升至$-200$）可进一步引导模型学习更保守的安全裕度。
    \item \textbf{增加训练多样性与持续时间：}当前1000轮训练在21架场景下可能未使模型充分收敛至安全最优策略。延长训练轮次并增加初始位置的随机化程度可能进一步提升CFCR。
\end{enumerate}

\subsection{DyGAT与安全距离参数的不对称贡献分析}

消融实验（表\ref{tab:ablation}）与敏感性分析（第4.6节）之间存在一个值得关注的不对称性：移除DyGAT后冲突暴增173.6\%，而安全距离$d_{\text{base}}$从40m变化至70m时冲突仅变化34.7\%。这意味着：

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{冲突风险的主导因素是图结构质量，而非安全距离的绝对数值。} 安全距离定义了``什么是冲突''的判定阈值，但不直接决定``如何避免冲突''的控制策略质量。DyGAT的图结构直接影响了模型对邻域风险的感知精度与决策质量——当图结构失能时（标准GAT），无论安全距离设为多少，模型都无法有效区分高风险与低风险的邻域飞行器对。
    \item \textbf{安全距离主要贡献于效率维度而非安全维度。} 敏感性分析（第4.6节）显示：$d_{\text{base}}$增大时冲突减少但延迟上升（安全-效率权衡），而$d_{\text{base}}$减小时延迟改善但冲突增多。这说明安全距离本质上是一个\emph{运营参数}（trade-off knob），而非\emph{技术创新}（capability enhancer）。这一认识并不否定动态安全距离的设计价值——与速度正相关的$d_{\text{safety}}$（式\ref{eq:dynamicsafety}）相比固定安全距离是一个有意义的改进——但建议将其在贡献声明中的权重调整至``工程环境设计''层面，而非与DyGAT、课程学习并列的技术创新。
    \item \textbf{这解释了为何CTDE-MARL方法之间在TC上的差异不显著（$p>0.05$）：}所有CTDE方法在21架密度下都学习了基本的冲突规避，TC的差异更多受安全距离参数、随机初始条件和探索噪声的影响，而非图结构质量的系统性差异。CFCR才是区分图结构质量的有效指标——它衡量的是``是否每次都能避得开''而非``平均避开了多少次''。
\end{enumerate}
...
面向工程化部署的实际需求，未来研究将从以下六个方向展开：

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{三维广域空域扩展：} 将模型迁移至三维空间，引入高度层分配、垂直起降动力学与多路口网络拓扑，研究区域级交通流协同优化。
    \item \textbf{与安全关键方法的系统基准对比：} 在当前仿真框架中实现MPC、ORCA/RVO与CBF等基线方法，建立包含形式化安全保证方法与学习型方法的全面性能基准，明确各类方法的适用边界与互补关系。
    \item \textbf{标准风谱模型的风场验证：} 采用Dryden风湍流模型（MIL-F-8785）或Kaimal谱模型（IEC 61400-1）替代当前均匀随机风扰，结合城市峡谷效应的CFD仿真数据，评估模型在真实低空湍流条件下的性能退化程度。
    ...
    \item \textbf{精细化的消融与超参数分析：} 分别独立移除DyGAT的空间注意力、时序注意力与动态边权重三个子组件，建立完整的组件级贡献排序；在$\lambda \in \{1.0, 1.5, 2.0, 2.5, 3.0\}$范围内系统评估轨迹加权系数的敏感性；统计冲突事件的严重程度分布（$d_{\min}/d_{\text{safety}}$直方图），建立CFCR与严重违规率的定量关系。
    \item \textbf{MARL+形式化安全的混合架构探索：} 将本文的学习型策略与ORCA/CBF等形式化安全保证方法进行架构级融合——由MARL负责效率优化与常规协同决策，由ORCA/CBF作为底层安全网提供实时冲突避免的硬性保证，探索``学习优化+形式化安全''双层架构对CFCR的提升潜力。
\end{enumerate}
